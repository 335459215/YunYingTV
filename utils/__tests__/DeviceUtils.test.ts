...
const createMockDimensions = (width: number, height: number) => ({
  width,
  height,
  scale: 1,
  fontScale: 1,
...
describe("DeviceUtils", () => {
  ...
  describe("getDeviceType", () => {
    it("应该在宽度 >= 1024 时返回 tv", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(1024, 768));
      expect(DeviceUtils.getDeviceType()).toBe("tv");
    });

    it("应该在宽度 >= 768 且 < 1024 时返回 tablet", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(768, 1024));
      expect(DeviceUtils.getDeviceType()).toBe("tablet");
    });

    it("应该在宽度 < 768 时返回 mobile", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(375, 812));
  ...
  describe("isTV", () => {
    it("应该在 TV 设备上返回 true", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(1920, 1080));
  ...
  describe("getOptimalSpacing", () => {
    it("应该为 mobile 设备返回基础间距 * 0.8", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(375, 812));
      expect(DeviceUtils.getOptimalSpacing(10)).toBe(8);
    });

    it("应该为 tablet 设备返回基础间距 * 1.0", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(768, 1024));
      expect(DeviceUtils.getOptimalSpacing(10)).toBe(10);
    });

    it("应该为 tv 设备返回基础间距 * 1.5", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(1920, 1080));
      expect(DeviceUtils.getOptimalSpacing(10)).toBe(15);
    });
  });

  describe("isLandscape", () => {
    it("应该在宽度大于高度时返回 true", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(812, 375));
      expect(DeviceUtils.isLandscape()).toBe(true);
    });

    it("应该在高度大于宽度时返回 false", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(375, 812));
      expect(DeviceUtils.isLandscape()).toBe(false);
    });

    it("应该在宽度和高度相等时返回 false", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(500, 500));
      expect(DeviceUtils.isLandscape()).toBe(false);
    });
  });

  describe("isPortrait", () => {
    it("应该在高度大于宽度时返回 true", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(375, 812));
      expect(DeviceUtils.isPortrait()).toBe(true);
    });

    it("应该在宽度大于高度时返回 false", () => {
      mockedDimensions.get.mockReturnValue(createMockDimensions(812, 375));
