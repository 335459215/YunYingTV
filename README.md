...
- 本项目不内置、不提供任何影视资源、节目源、解析服务、账号共享或破解能力；所有第三方接口、数据与媒体内容均由使用者自行配置。
- 项目运行过程中接入的第三方内容，其版权、商标及相关权利均归原权利人所有，本项目不主张任何相关权利。
- 使用者应自行确认接口来源、内容来源及使用方式符合所在地区法律法规与平台规则；因自行接入、传播或使用第三方内容产生的风险与责任，由使用者自行承担。
- 如相关权利人认为仓库中的说明、链接或兼容适配内容存在问题，可通过 Issue 联系处理。
